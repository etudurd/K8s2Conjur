...
**Save the output. As you can see, two API keys are generated: one for configuring the Ansible plugin to connect with the Conjur host (host:AnsibleConjurIntegration/AAP-node), and another for the automation itself to interact with the Conjur API and onboard the scanned Kubernetes deployments (user:AnsibleAutomationConjurUser@AnsibleConjurIntegration). **
